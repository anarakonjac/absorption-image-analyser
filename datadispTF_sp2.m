...
if viewAngle_sp2 == 0
    ...
    thermhorizsize = thermhorizsize_corrected/(2^0.5)*pixelsize2; % Conversion to rms width and microns
else
    thermhorizsize_corrected  = 1/cos(viewAngle_sp2)*(DGfit(4)^2 - DGfit(6)^2*sin(viewAngle_sp2)^2)^0.5; % Corrected for angle, still in px
    thermhorizsize  = thermhorizsize_corrected/(2^0.5)*pixelsize2; % Conversion to rms width  and microns
end

thermvertsize  = DGfit(6)/(2^0.5)*pixelsize2;
...
intrapthermvertsize = thermvertsize/sqrt(1+(2.0*pi*zfreq_sp2*tof_sp2*1.0e-3)^2);
intrapthermhorizsize = thermhorizsize/sqrt(1+(2.0*pi*xfreq_sp2*tof_sp2*1.0e-3)^2);
tempx =  mass*(2.0*pi*xfreq_sp2*intrapthermhorizsize*(2^0.5)).^2/(2*kB);
tempz = mass*(2.0*pi*zfreq_sp2*intrapthermvertsize*(2^0.5)).^2/(2*kB);
...
set(handles.text_back_h_sp2,'String',num2str(DGfit(1),'%6.2f'))
handles.back_h_sp2 = DGfit(1);
set(handles.text_amp_h_sp2,'String',num2str(DGfit(2),'%6.2f'))
handles.amp_h_sp2 = DGfit(2);
set(handles.text_cen_h_sp2,'String',num2str(DGfit(3),'%6.2f'))
handles.cen_h_sp2 = DGfit(3);
set(handles.text_sig_h_sp2,'String',num2str(DGfit(4)/2^0.5,'%6.2f'))
handles.sig_h_sp2 = DGfit(4)/2^0.5; % Converting to RMS width.

set(handles.text_back_v_sp2,'String',num2str(DGfit(1),'%6.2f'))
handles.back_v_sp2 = DGfit(1);
set(handles.text_amp_v_sp2,'String',num2str(DGfit(2),'%6.2f'))
handles.amp_v_sp2 = DGfit(2);
set(handles.text_cen_v_sp2,'String',num2str(DGfit(5),'%6.2f'))
handles.cen_v_sp2 = DGfit(5);
set(handles.text_sig_v_sp2,'String',num2str(DGfit(6)/2^0.5,'%6.2f'))
handles.sig_v_sp2 = DGfit(6)/2^0.5; % Converting to RMS width.

set(handles.text_aspecttof_sp2,'String',num2str(thermaspect,'%6.2f'))
